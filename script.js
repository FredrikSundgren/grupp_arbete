...
fetch("./images.json")
    ...
    .then((data) => {
        ...
        sort.addEventListener("change", (e) =>
            sortNew(e.target.value, originalArray)
        );
        ...
        function showProducts(images) {
            ...
            images.map((images) => {
                document
                    .querySelector(".shop-div")
                    .insertAdjacentHTML(
                        "beforeend",
                        ` <div class='img__block'> <img id=${images.id} src=${images.url}><h2>${images.title}</h2> <h3>Storlek: ${images.info.size}x${images.info.size1}cm Pris: ${images.info.price}kr</h3></div>`
                    );
                ...
                for (let i = 0; i < imgBlocks.length; i++) {
                    imgBlocks[i].addEventListener("click", (e) =>
                        productPopup(e.target.id)
                    );
                }
            });
        ...
        document
            ...
            .addEventListener("input", (e) => findTitle(e.target.value));
        ...
        function findTitle(title) {
            if (title != undefined) {
                ...
                document
                    ...
                    .insertAdjacentHTML(
                        "beforeend",
                        ` <div class='img__block'> <img id=${result.id} src=${result.url}><h2>${result.title}</h2> <h3>Storlek: ${result.info.size}x${result.info.size1}cm Pris: ${result.info.price}kr</h3></div>`
                    );

                document
                    .querySelector(".img__block")
                    .addEventListener("click", (e) => productPopup(e.target.id));
